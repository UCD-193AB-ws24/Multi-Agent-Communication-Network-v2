...
class Network_Manager():
    """
    Manages the network of nodes, handling communication and data updates.
    Attributes:
        node_list (list): List of nodes in the network.
        socket_sent (function): Callback function for socket communication.
        uart_sent (function): Callback function for UART communication.
        current_node (Node): The current node being processed.
    Methods:
        __init__():
            Initializes the Network_Manager with default values.
        add_node(name: str, address: int, uuid: bytes) -> Node:
            Adds a new node to the network.
        run_on_current_thread():
            Runs the network manager on the current thread, continuously.
        attach_callback(socket_sent, uart_sent):
            Attaches callback functions for socket and UART communication.
        getActiveNode() -> list:
            Retrieves a list of active nodes based on their status.
        getNodeData(data_ID: str, node_addr: int) -> bytes:
            Retrieves data from a specific node or all nodes.
        updateNodeData(node_addr: int, msg_payload: bytes):
            Updates the data of a specific node based on the provided payload.
        callback_socket(data: bytes) -> bytes:
            Handles socket callback logic and processes commands received via socket.
        callback_uart(data: bytes) -> bytes:
            Handles UART callback logic and processes commands received via UART.
    """
    def __init__(self):
    ...
    def getActiveNode(self):
        self.active_nodes = list(filter(lambda node: node.status == Node_Status.Active, self.node_list))
        return self.active_nodes
        
    def getNodeData(self, data_ID, node_addr: int):
...
# ============================= Socket Callback Logics =================================
    def callback_socket(self, data):
        ...
        if command == "ACT-C": # Count active node
            count = len(self.active_nodes) % 255 # will not exceed, just in case get "OverflowError" from encode
...
# }
        ...
        if "Data" in opcodes and op_code == opcodes["Data"]: # Data update
        ...
        if "Node Info" in opcodes and op_code == opcodes["Node Info"]: # Node Connected (become available) Update
            node_uuid = payload
            node_list = list(filter(lambda node: node.address == node_addr, self.node_list))
            if len(node_list) <= 0:
